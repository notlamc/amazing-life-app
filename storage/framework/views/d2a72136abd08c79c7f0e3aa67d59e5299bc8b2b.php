...
<script>
	$(document).ready(function(){
		...
		let table = $('#userTable').DataTable({
			ajax: "<?php echo e(route('superadmin.user.list')); ?>",
			columns: [
				...
				{ data: 'affiliate', name: 'affiliate', orderable: false, searchable: false },
				{
					data: 'id',
					render: function(id) {
						let url = "<?php echo e(route('superadmin.user.transactions', ':id')); ?>";
				...
				},
				{
					data: 'id',
					render: function(id) {
						let url = "<?php echo e(route('superadmin.user.subscriptions', ':id')); ?>";
						url = url.replace(':id', id);
		...
		$(document).on('change', '.toggle-status', function(){
			let id = $(this).data('id');
			$.ajax({ 
				url: "<?php echo e(route('superadmin.user.status')); ?>",
				type: "POST",
		...
		$(document).on('click', '.deleteUser', function(){
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					$.ajax({
						url: "<?php echo e(route('superadmin.user.delete')); ?>",
						type: "POST",
						data: { id: id, _token: "<?php echo e(csrf_token()); ?>" },
						success: function(res){
							if(res.success){
		...
		$(document).on('click', '.extend-btn', function(){
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					$.ajax({
						url: "<?php echo e(route('superadmin.user.extend')); ?>",
						type: "POST",
						data: {
...
<?php /**PATH F:\xampp\htdocs\amazinglifeapp\resources\views/admin/user/user-list.blade.php ENDPATH**/ ?>