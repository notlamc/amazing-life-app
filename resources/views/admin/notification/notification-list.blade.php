			...
			<div class="page-header">
				<div class="row align-items-center">
					...
					<div class="col-sm-6 text-end">
						<a href="{{ route('superadmin.notification.create') }}" class="btn btn-primary">
...
<script>
	$(document).ready(function(){
		...
		let table = $('#userTable').DataTable({
			ajax: "{{ route('superadmin.notification.list') }}",
			columns: [
				...
				{ 
					...
					render: function(id){
						return `
							...
							<a href="{{ url('superadmin/notification') }}/${id}/view" class="text-info me-2" title="View">
		...
		$(document).on('change', '.toggle-status', function(){
			let id = $(this).data('id');
			$.ajax({ 
				url: "{{ route('superadmin.notification.status') }}",
				type: "POST",
		...
		$(document).on('click', '.deleteVideo', function(){
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					$.ajax({
						url: "{{ route('superadmin.notification.delete') }}",
						type: "POST",
						data: { id: id, _token: "{{ csrf_token() }}" },
						success: function(res){
		...
		$(document).on('click', '.viewUser', function(){
			let id = $(this).data('id');
			$.ajax({
				url: "{{ route('superadmin.notification.details') }}",
				type: "POST",
				data: { id: id, _token: "{{ csrf_token() }}" },
				success: function(res){
					if (res.success) {
		...
		$(document).on('click', '.editVideo', function() {
			...
			let url = "{{ route('superadmin.notification.edit', ':id') }}";
