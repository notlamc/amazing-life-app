			...
			<div class="page-header">
				<div class="row align-items-center">
					...
					<div class="col-sm-6 text-end">
						<a href="{{ route('superadmin.category.create') }}" class="btn btn-primary">
...
<script>
	$(document).ready(function(){
		...
		let table = $('#userTable').DataTable({
			ajax: "{{ route('superadmin.category.list') }}",
			columns: [
		...
		$(document).on('change', '.toggle-status', function(){
			let id = $(this).data('id');
			$.ajax({ 
				url: "{{ route('superadmin.category.status') }}",
				type: "POST",
		...
		$(document).on('click', '.deleteCategory', function(){
			...
			Swal.fire({
				...
				if (result.isConfirmed) {
					$.ajax({
						url: "{{ route('superadmin.category.delete') }}",
						type: "POST",
						data: { id: id, _token: "{{ csrf_token() }}" },
						success: function(res){
		...
		$(document).on('click', '.editCategory', function() {
			...
			let url = "{{ route('superadmin.category.edit', ':id') }}";
